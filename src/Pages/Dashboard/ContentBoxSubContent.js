...
const CourseCard = (props) => {
    return (
        <div className={`w-full h-28 ${props.colorClass} p-4 flex items-stretch gap-3`}>
            <div className='w-16 h-full bg-white bg-opacity-40 flex items-center justify-center rounded'>
                <FontAwesomeIcon className='text-xl text-opacity-50 text-white' icon={props.icon} />
            </div>
            <div className='text-white flex flex-col justify-between'>
                <div>
                    <h3 className='mb-0 text-lg font-medium capitalize'>{props.heading}</h3>
                    <p className='capitalize text-sm'>{props.content}</p>
                </div>
